...
class RealtimeBase<T extends Opts> {
  ...
  private createEventHandlers(
    channel: string,
    historyOverride?: HistoryConfig | boolean
  ): RealtimeChannel<T> {
    const historyConfig = {
      ...this._history,
      ...
      return historyMessages
        .map((value) => {
          if (typeof value === "object" && value !== null) {
            const { id, channel, event, data } = value as {
              id: string;
              channel: string;
              event: string;
              data: unknown;
    ...
    const subscribeFunc = async <E extends EventPaths<T["schema"]>>({
      ...
      try {
        if (history) {
          const start =
            typeof history === "object" && history.start
              ? String(history.start)
              : "-";
          const end =
            typeof history === "object" && history.end
              ? String(history.end)
              : "+";
          ...
          for (const message of messages) {
            ...
            if (
              !typedMessage.event ||
              (events && !events.includes(typedMessage.event as E))
            )
              continue;
            ...
            if (result.success) {
              onData(
                result.data as unknown as EventPayloadUnion<T["schema"], E>
              );
            }
          }
          ...
          if (messages.length > 0) {
            lastHistoryId =
              (messages[messages.length - 1]?.id as string) ?? null;
          }
        }
    ...
    const findSchema = (path: string[]): z.$ZodType | undefined => {
      ...
      return typedCurrent?._zod || typedCurrent?._def
        ? (current as z.$ZodType)
        : undefined;
    };

    const emitFunc = async <K extends EventPath<T>>(
      event: K,
      data: EventData<T, K>,
